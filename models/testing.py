...
class TestUser(unittest.TestCase):
    ...
    def setUp(self):
        self.user = User("example@example.com", "password", "John", "Doe")

    def test_created_at_set_on_creation(self):
        self.assertLessEqual(datetime.now() - self.user.created_at, timedelta(seconds=1))

    def test_update_user_updates_updated_at(self):
        original_updated_at = self.user.updated_at
        self.user.update_user("newemail@example.com", "newpassword", "Jane", "Doe")
        self.assertGreater(self.user.updated_at, original_updated_at)
    def bussiness_rules(self):
        # Create User
